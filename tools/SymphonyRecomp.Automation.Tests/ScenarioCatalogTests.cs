...
public sealed class ScenarioCatalogTests
{
    ...
    public void M5ProbesUseTelemetryStageNamesAndHonestTransitionTimelines()
    {
        ...
        Assert.Equal("6", transition.Version);
        ...
        Assert.Equal(new[] { 60, 12, 55, 125 }, p1.Timeline.Select(value => value.Frames));
        Assert.Equal(new[] { 180, 8, 64 }, p2.Timeline.Select(value => value.Frames));
